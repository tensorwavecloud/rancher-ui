...
export default {
  ...
  computed: {
    ...
    generateLogoutRoute() {
      return this.isAuthLocalProvider
        ? { name: "auth-logout", query: { [LOGGED_OUT]: true } }
        : {
            name: "auth-logout",
            query: { [LOGGED_OUT]: true, [IS_SSO]: true },
          };
    },
    ...
    showAccountAndApiKeyLink() {
      ...
      const canSeeTokens = this.$store.getters["rancher/schemaFor"](
        NORMAN.TOKEN,
        false,
        false
      );
    ...
    showPageActions() {
      return (
        !this.featureRancherDesktop &&
        this.pageActions &&
        this.pageActions.length
      );
    },
    ...
    <div class="rd-header-right">
      ...
      <div v-if="extensionHeaderActions.length" class="header-buttons">
        <button
          ...
          type="button"
          class="btn header-btn role-tertiary"
          :data-testid="`extension-header-action-${
            action.labelKey || action.label
          }`"
      ...
      <div v-if="showPageActions" id="page-actions" class="actions">
        <i
        ...
        >
          <template slot="popover" class="user-menu">
            ...
            >
              ...
              >
                <a v-if="!a.separator" @click="pageAction(a)">{{
            ...
            >
              ...
              >
                <li
                  ...
                  @keypress.enter="navigate"
                >
                  <a :href="href" @blur="showMenu(false)">{{
