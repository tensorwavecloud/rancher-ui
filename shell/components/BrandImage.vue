...
export default {
  ...
  data() {
    const managementSettings = this.$store.getters["management/all"](
      MANAGEMENT.SETTING
    );

    const uiLoginBackgroundLight = managementSettings?.filter(
      (setting) => setting.id === SETTING.LOGIN_BACKGROUND_LIGHT
    )?.[0]?.value;
    const uiLoginBackgroundDark = managementSettings?.filter(
      (setting) => setting.id === SETTING.LOGIN_BACKGROUND_DARK
    )?.[0]?.value;
  ...
  computed: {
    ...
    brand() {
      const setting =
        this.managementSettings.filter(
          (setting) => setting.id === SETTING.BRAND
        )[0] || {};
    ...
    uiLogoLight() {
      const setting =
        this.managementSettings.filter(
          (setting) => setting.id === SETTING.LOGO_LIGHT
        )[0] || {};
    ...
    uiLogoDark() {
      const setting =
        this.managementSettings.filter(
          (setting) => setting.id === SETTING.LOGO_DARK
        )[0] || {};
    ...
    uiBannerLight() {
      const setting =
        this.managementSettings.filter(
          (setting) => setting.id === SETTING.BANNER_LIGHT
        )[0] || {};
    ...
    uiBannerDark() {
      const setting =
        this.managementSettings.filter(
          (setting) => setting.id === SETTING.BANNER_DARK
        )[0] || {};
