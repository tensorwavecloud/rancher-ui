...
export default {
  async fetch() {
    ...
    try {
      if (this.$store.getters["management/canList"](CATALOG.APP)) {
        this.apps = await this.$store.dispatch("management/findAll", {
          type: CATALOG.APP,
        });
      }
    } catch (e) {}
  ...
  computed: {
    ...
    brand() {
      const setting = this.globalSettings?.find(
        (gs) => gs.id === SETTING.BRAND
      );
    ...
    color() {
      const setting = this.globalSettings?.find(
        (gs) => gs.id === SETTING.PRIMARY_COLOR
      );
    ...
    cspAdapter: {
      handler(neu) {
        ...
        if (neu && !this.brand) {
          ...
          const brandSetting = this.globalSettings?.find(
            (gs) => gs.id === SETTING.BRAND
          );
          ...
          if (brandSetting) {
            ...
            if (url) {
              this.$store
                .dispatch("management/create", {
                  type: MANAGEMENT.SETTING,
                  metadata: { name: SETTING.BRAND },
                  value: "csp",
                  default: "",
                })
